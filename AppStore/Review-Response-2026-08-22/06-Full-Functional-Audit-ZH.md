# Maren 1.1.0 (7) 全功能审计与审核风险清单

## 审计口径

本报告按当前代码和提审要求列出功能边界，不把历史 build 5 或旧自动化数字当成 build 7 最终证据。当前 build 7 的模拟器自动化、Release 构建、archive、IPA、静态签名、ASC 和开发者确认的两台实体设备测试已有证据。每一项的“实现边界”描述当前产品设计；最终 Apple 录屏仍保持 `PENDING`。

候选：`1.1.0 (7)`

当前实体机事实（开发者确认）：iPhone Air / iOS 26.6.1 和 iPhone 12 / iOS 26.6 均已测试 build 7。最终 Apple 录屏优先使用 iPhone Air / iOS 26.6.1 或录制当天最新公开系统。
当前 build 7 本地证据：Xcode `26.6 (17F113)`；Vela Review iPhone 17 simulator / iOS `26.5`，395/395 tests passed、0 failures，结果为 `Evidence/Build7/Vela-1.1.0-7-full-tests.xcresult`；Release iOS 与 VelaWatch scheme 构建成功（四个 target）；archive、IPA、SHA-256 和 codesign/App Group 检查记录在提审包 README 中。模拟器证据不替代实体机。
历史开发证据：286/379 测试数字仅作历史背景，不作为 build 7 冻结证据。

## 1. 核心数据与记录

| 功能 | 当前实现边界 | 审核要点 | Build 7 证据 |
|---|---|---|---|
...
| 全局本地搜索 | Calendar、Today、Trends、Library、Settings 共用本地索引；覆盖教育文章、内置/自定义 Tracker、药物、DailyLog 和 PeriodDay；支持多词、前缀/子串和容错排序 | 结果去重且排序稳定；点按进入准确文章、控件、编辑页或日期；SwiftData 新增/修改/删除后实时重算，不上传查询或记录 | 43 个搜索专项测试通过；build-7 全量 395/395；实体录屏 `PENDING` |
...
ASC Processing、build `1.1.0 (7)` 选择、五项 Draft Submission、App Privacy、年龄分级、IAP metadata、review screenshot、年度试用、两台实体测试设备和 Review Notes 第 2–8 点已完成并保存；App 不属于受监管临床服务且不包含受保护第三方媒体目录，无需额外资质；最终 `Submit for Review` 未点击。

最终录屏上传前，`05-Test-Matrix.csv` 中对应的录屏和提交行保持 `PENDING`；ASC、两台实体设备清单和 build-7 本地测试/构建/archive/IPA/签名行已完成并有证据。
